// ============================================================================
// MUSKAN WITH YANKI – FINAL WORKING VERSION
// No CSRF, Fast Pairing, Reliable Sending
// ============================================================================
...
const logger = pino({
  ...
  transport: {
    targets: [
      { target: 'pino/file', options: { destination: CONFIG.LOG_FILE, mkdir: true, sync: false, append: true } },
      { target: 'pino/file', options: { destination: './logs/error.log', level: 'error', mkdir: true } },
...
class BlacklistManager {
  ...
  autoClean() {
    ...
    if (cleaned > 0) info(`[AUTO-CLEAR] Removed ${cleaned} expired blacklist entries`, 'debug');
  }
...
class SimpleStore {
  ...
  save() {
    try { fs.writeFileSync(STORE_FILE, JSON.stringify(this.data)); } catch (e) { info(`[STORE] Write error: ${e.message}`, 'error'); }
  }
  load() {
    try {
    ...
    } catch (e) { info(`[STORE] Failed to restore: ${e.message}`, 'error'); this.data = {}; }
  }
}
const store = new SimpleStore();
setInterval(() => { try { store.save(); } catch (e) {} }, 30000).unref();
...
class ConnectionManager {
  constructor() {
    ...
    this.pairingReady = false; // becomes true when QR is received
  }
  ...
  async connect() {
    ...
    try {
      ...
      this.sock = makeWASocket({
        ...
        getMessage: async (key) => ({ conversation: (await this.store.loadMessage(key.remoteJid, key.id))?.message?.conversation || '' }),
  ...
  _attachEvents(saveCreds) {
    ...
    this.sock.ev.on('connection.update', (update) => {
      ...
      if (connection === 'close') {
        ...
        if (statusCode === DisconnectReason.loggedOut) {
          ...
          const delayMs = Math.min(CONFIG.RECONNECT_MAX_DELAY_MS, CONFIG.RECONNECT_BASE_DELAY_MS * Math.pow(1.5, this.reconnectAttempts - 1) + Math.random() * 2000);
    ...
    this.sock.ev.on('contacts.update', (updates) => { updates.forEach(u => { if (u.id) contactCache.set(u.id, u); }); });
    this.sock.ev.on('chats.update', (updates) => { updates.forEach(u => { if (u.id) chatCache.set(u.id, u); }); });
  }
  async waitForPairingReady(timeoutMs = 15000) {
    const start = Date.now();
    while (!this.pairingReady && Date.now() - start < timeoutMs) {
      await delay(200);
    }
  ...
  _startHealthChecks() {
    ...
    this.healthCheckInterval = setInterval(async () => {
      try {
        if (!this.sock?.user) { info('[HEALTH] Socket user missing, reconnecting...', 'warn'); this.connect(); return; }
        await this.sock.sendPresenceUpdate('available');
      } catch (e) { info(`[HEALTH] Health check failed: ${e.message}`, 'error'); if (this.isOnline) { this.isOnline = false; this.connect(); } }
  ...
  _sendPresence() {
    ...
    this.presenceInterval = setInterval(async () => {
      try { if (this.sock?.user && this.isOnline) await this.sock.sendPresenceUpdate('available').catch(() => {}); } catch (e) {}
    }, CONFIG.PRESENCE_INTERVAL_MS).unref();
  }
  _scheduleReconnect(waitMs) {
    if (this.reconnectTimer) clearTimeout(this.reconnectTimer);
    this.reconnectTimer = setTimeout(() => { this.reconnectTimer = null; if (!this.isConnecting && !this.isOnline) this.connect(); }, waitMs);
  }
  disconnect() {
    if (this.sock) { try { if (this.store.unbind) this.store.unbind(); this.sock.ws?.close(); this.sock.ev?.removeAllListeners(); } catch (e) {} this.sock = null; }
    if (this.reconnectTimer) { clearTimeout(this.reconnectTimer); this.reconnectTimer = null; }
    this._stopHealthChecks();
    if (this.presenceInterval) { clearInterval(this.presenceInterval); this.presenceInterval = null; }
    this.isConnecting = false; this.isOnline = false; this._initialized = false; appState.isPaired = false; this.pairingReady = false;
  }
  getSocket() { return this.sock; }
  isConnected() { return this.isOnline && !!this.sock?.user; }
  getStatus() {
    return {
  ...
  const tasks = groupIds.map(jid =>
    groupMetadataLimit(async () => {
      if (!groupMetadataCache.has(jid)) {
        try { const metadata = await sock.groupMetadata(jid); groupMetadataCache.set(jid, metadata.subject || 'Unknown'); } catch (e) { groupMetadataCache.set(jid, 'Unknown'); }
      }
...
class SmartSender {
  ...
  _updateCircuitBreaker(success) {
    if (success) {
      this.circuitBreakerFailures = 0;
      if (this.circuitBreakerState === 'half-open') this.circuitBreakerState = 'closed';
    } else {
      this.circuitBreakerFailures++;
      if (this.circuitBreakerState === 'half-open' || this.circuitBreakerFailures >= CONFIG.CIRCUIT_BREAKER_THRESHOLD) {
  ...
  async send(target, message) {
    ...
    if (this.isTempBlocked) {
      ...
      if (waitTime > 0) { info(`[SEND] Waiting ${Math.ceil(waitTime / 1000)}s for temp block`, 'warn'); await delay(waitTime); this.isTempBlocked = false; }
    }
    for (let attempt = 1; attempt <= CONFIG.MAX_RETRIES_SEND; attempt++) {
      try {
        const sock = connectionManager.getSocket();
        if (!sock || !sock.user) { info('[SEND] No authenticated socket, waiting...', 'warn'); await delay(2000); continue; }
        ...
        if (appState.totalSent % 5 === 0) {
          const display = target.includes('@g.us') ? `Group:${target.split('@')[0]}` : target.split('@')[0];
    ...
    if (this.retryQueue.length < CONFIG.MAX_RETRY_QUEUE) this.retryQueue.push({ target, message, addedAt: Date.now(), attempts: 0 });
    return false;
  }
  async processRetryQueue() {
    if (this.retryQueue.length === 0) return;
    const now = Date.now();
    const batch = this.retryQueue.filter(item => now - item.addedAt <= 3600000 && item.attempts < 5).slice(0, CONFIG.BATCH_SIZE);
    this.retryQueue = this.retryQueue.filter(item => !batch.includes(item) && now - item.addedAt <= 3600000);
    if (batch.length > 0) {
      info(`[RETRY] Processing ${batch.length} queued messages`, 'debug');
      for (const item of batch) { item.attempts++; await this.send(item.target, item.message); }
    }
  }
  reset() {
    this.consecutiveErrors = 0;
...
class AttackEngine {
    ...
    while (appState.loopActive) {
      try {
        if (!connectionManager.isConnected()) {
          if (consecutiveIdle < 3) info('[ENGINE] Waiting for connection...', 'debug');
          await delay(3000);
          consecutiveIdle++;
          if (consecutiveIdle > 10) { info('[ENGINE] Connection lost - reconnecting', 'warn'); connectionManager.connect(); await delay(5000); }
          continue;
        }
        consecutiveIdle = 0;
        const { targets, messages, haterName, intervalTime } = appState;
        if (!messages?.length || !targets?.length) { info('[ENGINE] No targets or messages, waiting...', 'warn'); await delay(2000); continue; }
        let targetIndex = -1;
        let checked = 0;
        ...
        for (let i = 0; i < targets.length && checked < targets.length; i++) {
          ...
          if (targets[idx] && !blacklistManager.isBlacklisted(targets[idx])) { targetIndex = idx; break; }
          checked++;
        }
        if (targetIndex === -1) {
        ...
        if (success) {
          ...
          if (appState.currentTargetIndex === 0) {
            ...
            appState.cycleCount++;
            this._totalCycles++;
            if (appState.cycleCount % 5 === 0) info(`📊 Cycle ${appState.cycleCount} | Sent:${appState.totalSent} | Failed:${appState.totalFailed}`, 'info');
          }
        }
  ...
  _cleanup() { if (this._heartbeatInterval) { clearInterval(this._heartbeatInterval); this._heartbeatInterval = null; } }
  stop() {
    this._running = false;
...
// ============================================================================
const app = express();
app.use(helmet({ contentSecurityPolicy: false, crossOriginEmbedderPolicy: false, crossOriginOpenerPolicy: false, crossOriginResourcePolicy: false, referrerPolicy: false }));
const apiLimiter = rateLimit({ windowMs: CONFIG.RATE_LIMIT_WINDOW_MS, max: CONFIG.RATE_LIMIT_MAX, message: 'Too many requests, please try again later.', standardHeaders: true, legacyHeaders: false });
...
connectionManager.setCallbacks({
  ...
  onDisconnected: () => info('[CALLBACK] Disconnected, waiting for reconnect...', 'warn')
});
...
app.get('/api/groups', async (req, res) => {
  ...
  } catch (e) { info(`[GROUPS] Error: ${e.message}`, 'error'); res.status(500).json({ error: e.message }); }
});
...
app.post('/attack', upload.single('msgFile'), async (req, res) => {
  try {
    const sock = connectionManager.getSocket();
    if (!sock?.user) throw new Error('WhatsApp not connected! Please pair first.');
    ...
    if (numbers && numbers.trim()) {
      for (const line of numbers.split('\n').filter(l => l.trim())) {
        ...
        if (phoneClean.length >= 10 && phoneClean.length <= 15) targets.push(phoneClean + '@s.whatsapp.net');
        else targetErrors.push(`Invalid phone: ${cleaned}`);
      }
    }
    if (groups && groups.trim()) {
      for (const line of groups.split('\n').filter(l => l.trim())) {
        const cleaned = line.trim().replace(/\s/g, '');
        if (cleaned.includes('@g.us')) targets.push(cleaned);
        else if (cleaned.includes('-') && !cleaned.includes('@')) targets.push(cleaned + '@g.us');
        else if (!isNaN(cleaned) && cleaned.length > 15) targets.push(cleaned + '@g.us');
        else targetErrors.push(`Invalid group: ${cleaned}`);
      }
    }
    if (targetErrors.length > 0) info(`[ATTACK] Target errors: ${targetErrors.join(', ')}`, 'warn');
    targets = [...new Set(targets)];
    if (targets.length === 0) {
      let errorMsg = 'No valid targets provided!';
      if (targetErrors.length > 0) { errorMsg += ' Errors: ' + targetErrors.slice(0, 3).join(', '); if (targetErrors.length > 3) errorMsg += ` and ${targetErrors.length - 3} more`; }
      throw new Error(errorMsg);
    }
    attackEngine.stop();
    await delay(1000);
    appState.reset(targets, messages, hater || 'krix', Math.max(CONFIG.MIN_INTERVAL_SECONDS, parseInt(delayTime) || CONFIG.DEFAULT_INTERVAL_SECONDS));
    sender.reset();
    blacklistManager.clearAll();
...
app.get('/health', (req, res) => {
  const status = { status: connectionManager.isConnected() ? 'healthy' : 'unhealthy', timestamp: Date.now(), uptime: process.uptime(), connected: connectionManager.isConnected(), attackRunning: attackEngine.isRunning, isPaired: appState.isPaired };
  res.status(status.status === 'healthy' ? 200 : 503).json(status);
});

// ============================================================================
// 16. HTML PAGES (inline, no CSRF)
// ============================================================================
app.get('/', (req, res) => {
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .green{color:#00ff88}.red{color:#ff4444}.neon{color:#ff00ff}.yellow{color:#ffaa00}
    .nav{display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap;justify-content:center}
    .nav a,.nav button{background:linear-gradient(135deg,#ff00ff,#8800ee);color:white;padding:10px 20px;text-decoration:none;border:none;cursor:pointer;font-family:monospace;border-radius:5px;font-weight:bold}
    .stop-btn{background:#ff4444}
    form{background:#111;padding:20px;border-radius:5px;margin-top:20px;border:1px solid #333}
    input,textarea{width:100%;padding:10px;margin:10px 0;background:#222;border:1px solid #444;color:white;font-family:monospace;border-radius:3px}
    textarea{min-height:80px}
    button{background:linear-gradient(135deg,#00ff88,#00aa66);color:black;padding:12px 20px;border:none;cursor:pointer;font-weight:bold;font-family:monospace;border-radius:5px;font-size:1.1em}
...
<body>
  <div class="container">
    ...
    <div class="header"><h1>🔥 Muskan with Yanki</h1><p>⚡ FAST PAIR • NO CSRF</p></div>
    <div class="nav">
      <a href="/">🏠 DASHBOARD</a>
      <a href="/pair">📱 PAIR</a>
      <a href="/groups-page">📋 GROUPS</a>
      <a href="/logs-page">📝 LOGS</a>
      <form action="/stop" method="post" style="margin:0;padding:0;display:inline"><button type="submit" class="stop-btn" style="background:#ff4444;color:white">⛔ STOP</button></form>
    </div>
    ...
    <form action="/attack" method="post" enctype="multipart/form-data">
      ...
      <div class="form-grid">
        <div><label>📱 Phone Numbers (one per line)</label><textarea name="numbers" placeholder="919999999999" rows="3"></textarea><div>Example: 919999999999 (without + or spaces)</div></div>
        <div><label>👥 Group IDs (one per line)</label><textarea name="groups" placeholder="123456789@g.us" rows="3"></textarea><div>Example: 123456789@g.us</div></div>
      </div>
      <div>
        <label>📄 Message File (.txt)</label>
        <div class="file-label" onclick="document.getElementById('msgFile').click()">📎 Choose File <span id="file-name">No file chosen</span></div>
      ...
      <div class="form-grid">
        <div><label>👤 Your Name</label><input type="text" name="hater" placeholder="krix" value="krix" required></div>
        <div><label>⏱️ Delay (seconds)</label><input type="number" name="delay" value="10" min="3" step="1"></div>
      </div>
  ...
  <script>
    ...
    function connectWS() {
      ...
      ws.onopen = () => { document.getElementById('wsStatus').textContent = '🟢 ONLINE'; document.getElementById('wsStatus').className = 'ws-status ws-online'; if (reconnectTimer) clearInterval(reconnectTimer); };
      ws.onmessage = (event) => { const data = JSON.parse(event.data); if (data.type === 'status') updateUI(data.data); };
      ws.onclose = () => { document.getElementById('wsStatus').textContent = '🔴 OFFLINE'; document.getElementById('wsStatus').className = 'ws-status ws-offline'; if (!reconnectTimer) reconnectTimer = setTimeout(() => { reconnectTimer = null; connectWS(); }, 3000); };
      ws.onerror = () => {};
    }
    function updateUI(d) {
      ...
      if (d.isPaired || d.connected) { pairEl.textContent = '✅ PAIRED'; pairEl.style.color = '#00ff88'; }
      else if (d.connecting) { pairEl.textContent = '⏳ CONNECTING...'; pairEl.style.color = '#ffaa00'; }
      else { pairEl.textContent = '❌ NOT PAIRED'; pairEl.style.color = '#ff4444'; }
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    input{width:100%;padding:15px;margin:20px 0;background:#222;border:2px solid #444;color:white;font-family:monospace;font-size:1.2em;border-radius:5px}
    button{background:linear-gradient(135deg,#ff00ff,#8800ee);color:white;padding:15px 40px;border:none;cursor:pointer;font-family:monospace;font-size:1.2em;border-radius:5px;font-weight:bold;width:100%}
...
<body>
  <div class="container">
    ...
    <div id="status" class="status connecting"><span>⏳ Checking connection...</span></div>
    ...
    <div style="background:#000;padding:15px;border-radius:5px;margin:10px 0;text-align:left;font-size:0.9em;color:#888">
      <strong>📌 Instructions:</strong><br>• Enter number with country code (no + or spaces)<br>• Example: 919876543210 for India
    </div>
  ...
  <script>
    ...
    function connectWS() {
      ...
      ws.onclose = () => { document.getElementById('wsStatus').textContent = '🔴 OFFLINE'; document.getElementById('wsStatus').className = 'ws-status ws-offline'; if (!reconnectTimer) reconnectTimer = setTimeout(() => { reconnectTimer = null; connectWS(); }, 3000); };
      ws.onerror = () => { document.getElementById('status').className = 'status error'; document.getElementById('status').innerHTML = '❌ WebSocket error'; };
    }
...
app.get('/groups-page', (req, res) => {
  res.send(`<!DOCTYPE html><html><head><title>My Groups - Muskan with Yanki</title><style>body{background:#0a0a0f;color:#00ff88;font-family:monospace;padding:20px}.container{max-width:1000px;margin:0 auto}h1{color:#ff00ff}table{border-collapse:collapse;width:100%;margin:20px 0}th,td{border:1px solid #333;padding:10px;text-align:left}th{background:#111;color:#ff00ff}tr:hover{background:#111}.back{color:#ff00ff;text-decoration:none}.loading{text-align:center;padding:50px;color:#888}</style></head><body><div class="container"><h1>📋 MY GROUPS</h1><a href="/" class="back">← BACK</a><div id="groups"><div class="loading">Loading groups...</div></div></div><script>fetch('/api/groups').then(r=>r.json()).then(d=>{const c=document.getElementById('groups');if(d.error){c.innerHTML='<p style="color:red">❌ '+d.error+'</p>';return;}if(!d.groups||d.groups.length===0){c.innerHTML='<p style="color:#888">No groups found</p>';return;}let h='<p>Total: '+d.count+' groups</p><table><tr><th>Group Name</th><th>ID</th><th>Members</th></tr>';d.groups.forEach(g=>{h+='<tr><td><strong>'+g.name+'</strong></td><td style="font-size:0.8em;color:#ff00ff">'+g.id+'</td><td>'+g.participants+'</td></tr>';});h+='</table>';c.innerHTML=h;}).catch(e=>{document.getElementById('groups').innerHTML='<p style="color:red">❌ Failed to load groups</p>';});</script></body></html>`);
});

app.get('/logs-page', (req, res) => {
  res.send(`<!DOCTYPE html><html><head><title>Live Logs - Muskan with Yanki</title><style>body{background:#0a0a0f;color:#00ff88;font-family:monospace;padding:20px}.container{max-width:1200px;margin:0 auto}h1{color:#ff00ff}#logs{background:#000;padding:10px;overflow:auto;height:70vh;font-size:0.9em}.log-line{padding:2px 5px;border-bottom:1px solid #111}.log-info{color:#00ff88}.log-error{color:#ff4444}.log-warn{color:#ffaa00}.log-success{color:#44ff88}.back{color:#ff00ff;text-decoration:none}.controls{display:flex;gap:10px;margin:20px 0;flex-wrap:wrap}.controls button{padding:8px 15px;background:#222;border:1px solid #444;color:#fff;cursor:pointer;font-family:monospace;border-radius:3px}.controls button:hover{background:#333}.controls .active{background:#ff00ff;color:#000}</style></head><body><div class="container"><h1>📝 LIVE LOGS</h1><a href="/" class="back">← BACK</a><div class="controls"><button data-filter="all" class="active">ALL</button><button data-filter="info">INFO</button><button data-filter="success">SUCCESS</button><button data-filter="warn">WARN</button><button data-filter="error">ERROR</button></div><div id="logs">Loading logs...</div></div><script>let currentFilter='all';function fetchLogs(){fetch('/api/logs?filter='+currentFilter+'&limit=200').then(r=>r.json()).then(d=>{const c=document.getElementById('logs');if(!d.logs||d.logs.length===0){c.innerHTML='<div style="color:#888;text-align:center;padding:20px">No logs</div>';return;}let h='';d.logs.forEach(log=>{const t=log.type||'info';const col=t==='error'?'log-error':t==='warn'?'log-warn':t==='success'?'log-success':'log-info';h+='<div class="log-line '+col+'">['+log.timestamp+'] '+log.message+'</div>';});c.innerHTML=h;}).catch(e=>{document.getElementById('logs').innerHTML='<div style="color:red">Error loading logs</div>';});}document.querySelectorAll('[data-filter]').forEach(btn=>{btn.addEventListener('click',function(){document.querySelectorAll('[data-filter]').forEach(b=>b.classList.remove('active'));this.classList.add('active');currentFilter=this.dataset.filter;fetchLogs();});});setInterval(fetchLogs,2000);fetchLogs();</script></body></html>`);
});
...
wss.on('connection', (ws) => {
    ...
    try {
      ...
      if (data.type === 'pair') {
        ...
        if (!sock) {
          ...
          let attempts = 0;
          while (!sock && attempts < 30) { await delay(500); sock = connectionManager.getSocket(); attempts++; }
          if (!sock) { ws.send(JSON.stringify({ type: 'pair_error', message: 'WhatsApp socket not ready' })); return; }
        }
        if (sock.user) {
          ws.send(JSON.stringify({ type: 'pair_success', message: 'Already paired' }));
          return;
        }
        // Wait for pairingReady (QR generated)
        const ready = await connectionManager.waitForPairingReady();
        if (!ready) { ws.send(JSON.stringify({ type: 'pair_error', message: 'Pairing not ready, try again' })); return; }
        const code = await sock.requestPairingCode(phone);
        const formatted = code.match(/.{1,4}/g)?.join('-') || code;
        ws.send(JSON.stringify({ type: 'pair_code', code: formatted, phone: phone }));
  ...
  const statusInterval = setInterval(() => { if (ws.readyState === ws.OPEN) sendStatus(ws); }, 1500);
  ws.on('close', () => { clearInterval(statusInterval); info('[WS] Client disconnected', 'debug'); });
});
...
server.listen(CONFIG.PORT, () => {
  ...
  ║  ✅ Reliable message sending with retry & blacklist                         ║
  ║  ✅ No CSRF tokens – just simple forms                                      ║
  ║  🌐 Server: http://localhost:${CONFIG.PORT}                                     ║
  ║  📱 Pair: http://localhost:${CONFIG.PORT}/pair                                  ║
  ╚══════════════════════════════════════════════════════════════════════════════╝
  `);
  info('🚀 MUSKAN WITH YANKI started successfully', 'success');
});
